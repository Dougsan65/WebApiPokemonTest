...
async function showPokemons() {
  ...
  for (let i = start; i < end; i++) {
    ...
    imgPoke.innerHTML = `<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${
      i + 1
    }.png" alt="${pokemonNome}">`;
  ...
  document.getElementById(
    "pageCounter"
  ).innerText = `Página ${ActualPage} de ${totalPages}`;
}
...
document.addEventListener("click", function (event) {
  ...
  if (
    target.classList.contains("pokemon-item") ||
    target.parentElement.classList.contains("pokemon-item")
  ) {
    let pokemonName;
    if (target.classList.contains("pokemon-item")) {
      ...
      pokemonName =
        target.parentElement.querySelector(".pokemon-name").innerText;
    }
    if (pokemonName) {
      window.open(
